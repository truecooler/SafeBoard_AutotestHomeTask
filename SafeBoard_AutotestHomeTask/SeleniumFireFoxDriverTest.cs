	...
	public class SeleniumFireFoxDriverTest
	{
		...
		public void VK_LoginThenLikeAndCommentEugene_ExceptionShouldNotOccur()
		{
			using (var driver = new FirefoxDriver(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)))
			{
				...
				driver.FindElement(By.XPath("//*[starts-with(@id,'reply_field')]")).SendKeys("желаю успехов");//набираем ему комментарий под фото
		...
		public void VK_LoginThenLikeAndCommentFirstFriend_ExceptionShouldNotOccur()
		{
			using (var driver = new FirefoxDriver(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)))
			{
				...
				driver.FindElement(By.XPath("//*[starts-with(@id,'reply_field')]")).SendKeys("хорошо выглядишь");//набираем ему комментарий под фото
